...
class Mat{//材質
    ...
    Shading(lightinfos,hit){
        ...
        for(let i=0;i<lightinfos.length;i++){
            ...
            let fd = light * view * (1.0 / Math.PI);//アルベド反射成分の導出
            ...
            //let F=this.metalic+(1.0-this.metalic)*(Math.pow(1.0-Math.abs(vh),5));//F項の導出(Schlick近似)
            ...
            //let fs=(D*G*F)/(4*Math.abs(ln*vn));//スペキュラ反射成分の導出
            let fs=F.mult((D*G)/(4*Math.abs(ln*vn)));//スペキュラ反射成分の導出(Vector3)
            let color=this.diffuseColor.mult(fd).add(fs).multvec(lightinfos[i].color);
            shadingcolor=shadingcolor.add(color);
            //console.log(D);
            //shadingcolor=shadingcolor.add(lightinfos[i].color.mult(fs));//シェーディング色に反映
        }
        return shadingcolor;
    }
    ...
    updateUI(){
        ...
        this.UI.innerHTML +="<br>";
        this.UI.innerHTML += 'Smoothness<input name="Smoothness" type="range" name="num" min="0" max="0.9" step="0.1" value="'+this.smoothness+'">';
        this.UI.innerHTML +="<br>";
        ...
        this.SpecularColor=this.albedo.mult(0.04 * (1.0 - this.metalic)+this.metalic);//メタリックに合わせた鏡面反射の成分の更新
        /*1-smoothness(√α)の2乗と4乗はシェーディングの上で頻出するので先に計算しておく*/
        this.a=Math.pow(1-this.smoothness,2);
        this.aa=Math.pow(1-this.smoothness,4);
    }
